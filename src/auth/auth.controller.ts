import { Controller, Post, Body, UseGuards, Request as ReqDecorator, Get, Req, HttpException, HttpStatus } from '@nestjs/common';
...
export class AuthController {
  ...
  @Post('link-phone')
  @ApiOperation({ summary: 'Links a phone number to the authenticated user\'s account' })
  @ApiBody({ type: LinkPhoneDto })
  @ApiResponse({ status: 200, description: 'Phone number linked successfully.' })
  @ApiResponse({ status: 400, description: 'Bad request or phone number already exists.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async linkPhone(@ReqDecorator() req: AuthenticatedRequest, @Body() linkPhoneDto: LinkPhoneDto) {
    return this.authService.linkPhoneNumber(req.user.userId, linkPhoneDto.phoneNumber);
  }

  @UseGuards(JwtAuthGuard)
  @Post('link-email')
  @ApiOperation({ summary: 'Links an email to the authenticated user\'s account and sends verification email' })
  @ApiBody({ type: LinkEmailDto })
  @ApiResponse({ status: 200, description: 'Verification email sent successfully.' })
  @ApiResponse({ status: 400, description: 'Bad request or email already exists.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async linkEmail(@ReqDecorator() req: AuthenticatedRequest, @Body() linkEmailDto: LinkEmailDto) {
